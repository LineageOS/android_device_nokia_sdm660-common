...
PATCH_ELF="$TOOLS_LINEAGE"/patchelf
...
function blob_fixup() {
    ...
    vendor/lib64/hw/camera.sdm660.so)
        "$PATCH_ELF" --remove-needed "libMegviiFacepp.so" "${2}"
        "$PATCH_ELF" --remove-needed "libmegface-new.so" "${2}"
        "$PATCH_ELF" --add-needed "libshim_megvii.so" "${2}"
        ;;
    esac
}
